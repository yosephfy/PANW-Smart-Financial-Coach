      ...
      {!!categoryData.length && (
        <div className="border border-slate-700 rounded p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs text-slate-400">MTD sample (last fetch)</div>
          </div>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={categoryData}>
                <XAxis dataKey="name" tick={{ fill: '#cbd5e1', fontSize: 12 }} interval={0} angle={-15} textAnchor="end" height={50} />
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-slate-700/60">
                ...
                <td className={`px-3 py-2 whitespace-nowrap ${r.amount < 0 ? 'text-red-300' : 'text-green-300'}`}>{fmtCurrency(r.amount)}</td>
                <td className="px-3 py-2">
                  {r.category ? <Badge variant={categoryVariant(r.category)}>{r.category}</Badge> : <span className="text-slate-500">-</span>}
                </td>
                <td className="px-3 py-2">
                  {r.category_source ? <Badge variant="info" title={r.category_provenance || undefined}>{r.category_source}</Badge> : <span className="text-slate-500">-</span>}
                </td>
