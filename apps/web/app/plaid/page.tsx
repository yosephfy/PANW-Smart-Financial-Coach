...
export default function PlaidPage() {
  ...
  const [items, setItems] = useState<
    { item_id: string; institution_name?: string; created_at?: string }[]
  >([]);
  ...
  const loadItems = async () => {
    if (!userId) return;
    const r = await fetch(
      `${API}/plaid/items?user_id=${encodeURIComponent(userId)}`
    );
    const j = await r.json();
  ...
  const deleteItem = async (item_id: string) => {
    if (!userId) return;
    await fetch(
      `${API}/plaid/items/${encodeURIComponent(
        item_id
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-slate-400 text-sm">
        Tip: In Sandbox, pick Test credentials in Plaid Link, then use the
        Import button to pull transactions into the database.
      </p>
                ...
                <tr key={i.item_id} className="border-t border-slate-700/60">
                  ...
                  <td className="px-3 py-2">
                    <button
                      ...
                      className="px-2 py-0.5 rounded bg-red-700 text-white text-xs"
                    >
                      Delete
                    </button>
                  </td>
